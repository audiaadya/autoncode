...
std::string skills(bool calibrate, mik::info_variation var, bool get_name) {
    ...
    chassis.drive_distance(10, {.min_voltage = 12, wait = false});
    /* example of something i could implement later once auton is tested
        chassis.wait_until_within(10); waits until the robot is within 10 inches of the target distance before continuing
        and then chassis.cancel_motion(); would cancel the motion and allow the robot to continue with the rest of the auton
        motion chaining not the best for consistent autons but for speed is best

        swerving by using move_to_pose and then using the lead and drift parameters to make the robot move in a boomerang motion to avoid obstacles
    */
